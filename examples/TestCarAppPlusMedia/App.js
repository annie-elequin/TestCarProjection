...
/**
 * Example: Car App + media.
 * - mediaOnly: false, mediaSupport: true → Car App Service and MediaBrowserService both added.
 * - App shows custom Car App UI on Android Auto and participates in the media/now-playing slot.
...
export default function App() {
  ...
  useEffect(() => {
    CarProjection.registerScreen({
      ...
      template: createListTemplate({
        title: 'Car App + Media',
        header: 'Template UI + Now Playing',
        items: [
          { title: 'Item 1', texts: ['Car App and MediaBrowserService'] },
          { title: 'Item 2', texts: ['Play a track to see now-playing on car'] },
        ],
      }),
    });
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>TestCarAppPlusMedia</Text>
      <Text style={styles.subtitle}>Car App + Media (template UI + now-playing)</Text>
      {Platform.OS === 'android' && (
        <Text style={styles.hint}>Connect to Android Auto: Car App UI and media slot both active.</Text>
      )}
